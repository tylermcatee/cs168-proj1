...
class LearningSwitch (api.Entity):
  ...
  def handle_rx (self, packet, in_port):
    ...
    # Remember that for the packet.src, it came in on in_port
    self.dst_to_port[packet.src] = in_port

    # If we know where to send the dst, send it directly
    if packet.dst in self.dst_to_port:
      out_port = self.dst_to_port[packet.dst]
